...
Owns no artifact of its own: no report file, no spec, no test suite. Chat output only for its verdict (plus screenshots/logs saved to the scratch area). Does not write code (`/develop`), tests (`/test`), or context files (`AGENTS.md`/`CLAUDE.md`).

Explicit exception, closing gate ticks only, on files other skills own: on PASS, tick the `Verify it` box in the feature's `docs/scope/` row (owned by `/scope`); tick, in the feature's `verify.md` (owned by `/develop`), each step actually run and passed; and, only once the engineer confirms `done`, flip the governing spec's `**Status**:` line from `In Progress` to `Accepted` (owned by `/architect`). Each of these is one existing line flipped, never new prose or a rewrite; see Step 5.
...
<!-- BASE-BRANCH-RESOLUTION:START (identical in check/modes/review.md; edit both or neither) -->
...
4. Still nothing (a brand new repo with no commits on any candidate): there is no base to diff against yet; scope to uncommitted changes only and say so in the report rather than guessing a branch name.
<!-- BASE-BRANCH-RESOLUTION:END -->
List changed files: `git diff --name-status "$BASE"...HEAD`, `git diff --name-status HEAD`, and `git ls-files --others --exclude-standard`.
