...
Write `test-preferences.json` at the resolved `packageRoot` (from `SKILL.md` Step 1b's monorepo resolution): the project root itself in the common single-project case, but one file per package root in a monorepo, per the Q0 monorepo rule above — never at the repo root when packages are scoped separately, or one package's runner and gate leaks onto its siblings. It has two shapes, and which one you write depends on what the engineer chose at Q0.
