...
- `test-preferences.json` at the resolved `packageRoot` (the project root in the common single-project case; one file per package root in a monorepo, never a single shared file at the repo root), created and maintained by this skill
...
Read `test-preferences.json` at each scoped file's resolved `packageRoot` from Step 1b (the project root itself in the common single-project case; one file per package root in a monorepo — a package never inherits, reads, or writes a sibling's or the repo root's preferences) (file tool; "not found" = no prefs). Branch on whether it names a `tool`, not on whether the file exists:
...
Check the chosen unit tool, E2E tool (if any), and addon (if any) with file tools. **The manifest declaration is what counts, not mere presence on disk:** a package can sit under `node_modules` as someone else's transitive/hoisted dependency without ever being declared for this project, and a clean install (`npm ci`, a teammate's fresh clone, CI) reproduces only what the manifest and lockfile declare, not whatever happens to exist in the current `node_modules`. Checking `node_modules` alone can pass locally today and fail in CI or another agent's session tomorrow.
- JS/TS: **declared in `package.json`** (`dependencies` or `devDependencies`) is the check; confirm it further via the lockfile (`package-lock.json`/`pnpm-lock.yaml`/`yarn.lock`) and `node_modules/<pkg>` if you want extra certainty it actually resolved, but a `node_modules/<pkg>` hit with no `package.json` entry is still **missing**, not installed.
- Python: **declared in `pyproject.toml`/`requirements.txt`** is the check; `pip show <tool>` (where available) confirms it resolves, but a resolvable import with no manifest entry is still missing for a clean install.
- Go: **declared in `go.mod`** (a direct requirement) is the check, not `go.sum` alone (`go.sum` records checksums for every transitive dependency already resolved, whether or not this project's code depends on it directly).

All declared → Step 6. Any missing (not declared, whatever `node_modules`/local state shows) → confirm first:
