...
<!-- FEATURE / ENHANCEMENT modes only (a spec building to a scope feature). ARCHITECTURE and
     CROSS-CUTTING decision-only specs omit this section entirely: no meta acceptance criteria
     like "spec records the stack" — the decision section (## Proposed stack / ## Standard
     definition) is the contract instead. See "Decision-only spec layout" below. -->
## Requirements
...
<!-- FEATURE / ENHANCEMENT modes only. ARCHITECTURE and CROSS-CUTTING decision-only specs omit
     this section entirely: no implementation steps here. The steps that execute the decision
     belong to the feature that runs it (e.g. a stack decision's scaffold sub task) and are
     derived by /develop at build time — writing them here specs the same work twice. See
     "Decision-only spec layout" below. -->
...
- **Build spec** (what `/develop` reads to build): **`## Requirements`** (the acceptance criteria contract), **`## Decision`**, the design/spec section (**`## Feature design`** for a FEATURE spec, **`## Proposed stack`** for an ARCHITECTURE spec, or the equivalent spec table, e.g. `## Standard definition`), **`## Build plan`** (the ordered tasks derived from the surface + acceptance criteria), and **`## Consequences`** (the constraints the build must honor). This is the WHAT, the implementable spec. The **acceptance criteria in `## Requirements` are the contract `/develop` builds to and `/check verify` checks.** **Decision-only** (ARCHITECTURE / CROSS-CUTTING): `## Requirements` and `## Build plan` are omitted, not left empty — the decision section (`## Proposed stack` / `## Standard definition`) is the contract on its own; see "Decision-only spec layout" below.
...
## Decision-only spec layout (ARCHITECTURE / CROSS-CUTTING)

These two modes record a decision, not an implementation plan (per "Decision only specs" in `agent-prompt.md`'s Expert rules). **Omit `## Requirements` and `## Build plan` entirely** — no meta acceptance criteria like "spec records the stack," no pre-written implementation steps. The decision section stands in as the contract on its own. Section order for a decision-only spec:
...
5. `## Decision`
6. `## Rationale`
7. The decision section, immediately after Rationale, same placement `## Feature design` uses in a FEATURE spec: `## Proposed stack` (ARCHITECTURE) or `## Standard definition` (CROSS-CUTTING)
8. `## Consequences`
9. `## Follow-up` (optional)
10. `## References` (gated by `REFERENCES_LEVEL`, as elsewhere)

The feature that executes this decision (for a stack decision, its scaffold sub task) derives its own build steps from the decision section at build time (`/develop`); do not write those steps here, or the same work ends up specced twice.
