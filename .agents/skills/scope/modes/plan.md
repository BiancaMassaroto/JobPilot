...
Detect (skip `node_modules/` and `.git/`): source files (any `.ts`, `.tsx`, `.js`, `.py`, `.go`, `.rs`), manifests, or an existing scope under `docs/scope/` (`scope.md`, or `index.md` + epic files; monorepo: `docs/scope/<workspace>/`); any present ⇒ brownfield, none ⇒ greenfield; root `AGENTS.md`, noting the shape.
