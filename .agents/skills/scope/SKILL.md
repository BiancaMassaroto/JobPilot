...
Any Agent Skills client on macOS, Linux, Windows. Detection snippets are POSIX reference; use your agent's cross platform file tools. Planning runs inline; the two subagents below (Step 1 brownfield code scan, Step 6b sourcing) are optional and capability first, degrade to inline. Capability first on every panel too, per the decision panel rule above: use the agent's picker (`AskUserQuestion` on Claude Code) where one exists; only a client with no interactive picker falls back to the same options as plain text.
