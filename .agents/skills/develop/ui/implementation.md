...
```
**Built <name> (<file paths>) · <full product surface | component>, <accessibility claim>.**
...
**The accessibility claim is conditional, never a default "WCAG AA, self check passed."** Say that line only when Phase 6 actually ran every required check for this build: the disqualifier audit, AND, where the platform gave you a render surface (browser, screenshot, simulator/preview), the visual look pass. No render capability this session → you have not verified contrast, focus visibility, or layout by eye, whatever the code implies; claim `built to spec, accessibility unverified (no render capability this session)` instead, and repeat it under Heads up so it isn't missed. Phase 6 found an issue you could not fix → claim `WCAG AA audited, N issue(s) open` and list them under Heads up. Only a build that actually completed the disqualifier audit and, when possible, the render check earns the unqualified `WCAG AA, self check passed`.

Say it plainly if the self check found and fixed a defect, if a token/asset issue needs a manual look, or if a required check above didn't run (report it as unverified, per the claim rule); otherwise do not list the passing checks beyond the headline claim itself (semantic HTML, keyboard, screen reader, tokens are the guide's bar, not a separate report field). The design source, stack, fonts, and token file are recorded in `design.md` and the code, not here. `/test` reads the acceptance criteria and `verify.md`, so it needs no "what to verify" list in this summary.
