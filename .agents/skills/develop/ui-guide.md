...
**Nothing installed yet for one of these (styling system, navigation library, icon set) is a load bearing gap, not a detail to invent here.** A styling system, a navigation library, and an icon set are each exactly the kind of "library" choice `SKILL.md` Step 0 gates on; do not rationalize it as "just wiring" because it looks small, and never silently pick one, note it in the report, and move on. Follow the same load bearing gap handling `flow/build.md` Step 2 defines for a gap found mid build: a deliberated governing spec exists → route to `/architect <feature>: pick the <styling system | navigation library | icon set>` and resume once it's recorded; the governing spec is already `Assumed` → append the pick to its `## Assumption built on` (the one write `/develop` owns) and build; no spec at all (Step 0 was judged "no real decision") → the decision is owed after all, go back to the Step 0 panel (`Architect it first`, or `Build now, record it as an assumed spec`, folding this choice in) before writing UI code against it.
