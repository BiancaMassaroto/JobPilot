...
**InsForge is mandatory, not the judgment call below.** If this sub task writes or edits any InsForge SDK or backend integration code (auth, database, storage, AI, functions, realtime, payments), call the `fetch-docs` or `fetch-sdk-docs` MCP tool for the relevant feature/language **before** touching that code, every time, no matter how stable or familiar the API looks; this is a hard project rule (root `AGENTS.md`), not a "genuinely need it" call. Skipping it because the stack "looks stable" is exactly the failure this rule exists to prevent. If neither MCP tool is available this session, do not silently build from memory: say so plainly in the Step 4 report ("InsForge doc tools unavailable this session, built from memory, verify against current docs before shipping") and flag it, never proceed as if the check ran.

**Every other tool (only when needed): offload current usage lookups to a web subagent.** Only when you genuinely need the current usage/API of a tool the spec already decided (fast moving or newly released) and you're unsure your knowledge is current. Most builds don't need it: on a stable stack, build from knowledge and let the typecheck/build/lint loop catch a stale API. Never to choose or reconsider a tool (`/architect`'s job): look up *how to use* the decided tool, not *whether*; if docs reveal it can't work, that's the "spec is wrong" path (Step 3), not a silent swap. Not a second fetch of the spec's reference links (human facing, verified at design time).
...
**Task source, one owner of progress.** Governing spec → its `## Build plan` is the atomic checklist (tasks tagged with their AC; the data model migration sized to the feature, one or sliced per slice) and the single source of truth for build progress: build from it and tick `[ ]` → `[x]` there as each task lands, checkbox state only, per Artifact ownership in `SKILL.md`. The scope carries only a coarser milestone rollup under the feature's `Build it: /develop <feature>` box (2 to 5 sub items, updated per Step 4), a summary of the same ticks, never a second place progress is tracked; `Verify it` and `Test it` belong to `/check verify` and `/test`. No spec → the scope checkboxes are the tasks, and then they are the one owner.
...
- **Tick atomic tasks in the spec (the one owner of progress), the coarser rollup in the scope** (only what you verified built, only in the Step 0 scope file): each completed `## Build plan` task, checkbox state only, in the spec; a milestone sub box when its spec tasks are done; the `Build it` box when all milestones are done; fill the pointer line (`code in <path>`). No spec feature → tick its scope checkbox(es) directly, and they are the one owner instead. **Closing gate: report what you ticked in each file** (scope boxes + spec tasks/status), or say no scope row matched; never finish silently.
- **`done` follows the workflow tier gate; `/develop` may set it only at `Prototype`.** Tick what you built. Once it's landed and self checked (typecheck/build green; UI rendered if you could; migration applied and schema live), determine the effective tier: the feature's own tier tag if set, else the project `**Workflow:**` default. **`Prototype`** (the feature carries no `Verify it`/`Test it` box; build plus its own self check is the whole gate, the engineer opted out of separate verification) → offer to mark it `done` now; the engineer's confirmation sets it, that part is their call. **`Alpha`/`Beta`/`GA`** (the feature carries a `Verify it` and/or `Test it` box) → `/develop` never sets `done` here, even if the engineer asks: leave the feature `in-progress`, name the exact next command that closes it (`/check verify <feature>`, then `/test <feature>` if the tier has it), and say plainly that those boxes, not `/develop`, gate `done` at this tier. An `Assumed` governing spec doesn't change this by tier: flag it ("built on an unratified decision, `/architect` to ratify when you can") regardless of whether `done` is offered now or later.
...
- **Suggest the next step from the scope; make clear it is optional, but never as an alternative to a box the workflow tier still requires.** At `Prototype` (no `Verify it`/`Test it` box), e.g. "Built and self checked. Mark it done, or move to the next thing, your call." At `Alpha`/`Beta`/`GA` (a `Verify it`/`Test it` box remains), the suggested command is the way to close it, not one option among which "mark it done" is another, e.g. "Suggested next: `/check verify <feature>`, or move to the next thing and come back to close this out later." The suggestion is this feature's **first unticked box**; all boxes ticked → point to the next feature. A feature with no `Verify it`/`Test it` box has none to suggest, just move on. Never present a stage as required, and never suggest a stage whose box is not on the feature.
