...
2. Root `AGENTS.md` (the canonical file) missing: before picking a root mode, check the same `WORKFLOW_SETUP` signal SKILL.md's pre-flight table uses — root missing while the workflow already decided the stack (a `docs/specs/` architecture spec with a `## Proposed stack` section, and/or a `docs/scope/` "Stack and architecture" scaffold feature). A freshly scaffolded workflow project has a manifest and scaffold source already, same as real brownfield code, but it is still greenfield; skipping this check routes it into a whole-repo scan that never asks the coding standards questions and writes root from the wrong signal.
   - `WORKFLOW_SETUP` fires → run the greenfield phase (`modes/greenfield.md`) fully first (ask the coding standards questions, seed root `AGENTS.md` + its `CLAUDE.md` pointer from the answers and the spec's stack), then continue with the area scan.
   - No `WORKFLOW_SETUP`, and no legacy `CLAUDE.md` to migrate → run the whole-repo scan (`modes/whole-repo.md`) fully first (write root AGENTS.md + CLAUDE.md pointer), then continue with the area scan.
   - Only a legacy root `CLAUDE.md` → run the legacy migration first.
   - Root `AGENTS.md` exists → proceed directly.
