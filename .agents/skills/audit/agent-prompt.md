...
Durable context goes in `AGENTS.md` (root or `<area>/AGENTS.md`), tool agnostic, read by every agent. For each `AGENTS.md`, create a sibling `CLAUDE.md`. Its complete generated content is exactly:
...
`DEFERRED_TOOLS_OR_NONE` (the engineer picked `Not now, later` in the tool sweep, so nothing was searched, installed, or declined) writes **nothing** into `AGENTS.md`, curated or otherwise; there's no decision yet to record. Carry it forward instead to the Report format below as a Heads up line, so the deferred state doesn't just vanish once this session ends.
...
**Step 3: Create root AGENTS.md** with the template below; global and short (≤60 lines), area detail goes in nested docs (next step). Apply `INSTALLED_SKILLS_OR_NONE` / `MCP_SERVERS_OR_NONE` / `DECLINED_TOOLS_OR_NONE`, and `DEFERRED_TOOLS_OR_NONE` if provided, exactly as GREENFIELD Step 2 above describes (the `## Agent skills` bullets, the compact `MCP servers:`/`Declined:` lines, and the deferred state writing nothing to the file but reaching the final report) — carrying fewer than all three of the first group loses that tool state from the file this run produces, and a later run offers an already installed or declined tool again.
...
<The engineer's recorded git integration preference (from the greenfield git integration
 question), not a mirrored source of truth: `/develop` reads it to branch and commit,
 `/document` to gate the PR. Absent means off.
 - integration: on | off
 - branch prefix: <e.g. feat/> (only if integration: on)
 - commit: per-milestone | end-of-build | manual (only if integration: on)
 - co-author: <exact Co-Authored-By trailer text, optional> (only if the engineer wants a fixed
   trailer regardless of which client commits; absent → `/develop` detects the active client/model
   per commit instead, since this skill runs on Codex and other clients too, not just Claude Code)>
...
- Tool candidates not searched yet: <DEFERRED_TOOLS_OR_NONE, one line>   (greenfield or whole-repo only, when the engineer picked `Not now, later`)
```
