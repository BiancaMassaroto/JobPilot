...
**Canonical file:** durable context lives in the tool agnostic **`AGENTS.md`**; **`CLAUDE.md` is only a pointer** importing its sibling AGENTS.md via Claude Code's `@` directive. Never add project context to an existing CLAUDE.md, and never rewrite an existing AGENTS.md wholesale. When you create a new nested `AGENTS.md`, also create its sibling `CLAUDE.md` containing only:
...
- **Scope evidence files (test/tooling changes, not durable conventions)**: SCOPE_EVIDENCE_FILES
...
> Step 1's source file filtering (dropping `*.test.*`, `docs/**`) governs what you sync AGENTS.md from; it does not limit reconciliation. Use `SCOPE_EVIDENCE_FILES` (the test/tooling changes Step 1 kept aside for exactly this) as a starting signal, then inspect test files, AGENTS.md, and config directly to judge completion; a run with empty `CHANGED_FILES` but non-empty `SCOPE_EVIDENCE_FILES` still reaches this step and must reconcile.
