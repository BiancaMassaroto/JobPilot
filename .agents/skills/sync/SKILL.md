...
**Freshness first (teams):** `git fetch --quiet`; then, only if `origin/$BASE` actually exists (`git rev-parse --verify origin/$BASE` succeeds; no remote at all, or `$BASE` was never pushed, just skip this check), run `git rev-list --count HEAD..origin/$BASE`. If it's > 0 you are behind `origin/$BASE`, warn the engineer to pull first, a teammate may have already synced these docs.
...
Remove duplicates, then split into two evidence sets. Keep them separate: AGENTS.md sync needs a durable area convention, scope reconciliation only needs proof a task landed, and tests or tooling config are exactly that proof even though they carry no convention. Conflating the two into one filter means a test only or tooling only change would never reach scope reconciliation.

- **`AGENTS_SOURCE_FILES`** (sync AGENTS.md *from* these): drop documentation (`AGENTS.md` at any level, `docs/**`, `*.md`), test files (`*.test.*`, `*.spec.*`, `__tests__/`, etc.), lock files, and generated output; none of these carry a durable area convention.
- **`SCOPE_EVIDENCE_FILES`** (kept for Step 6's sub task reconciliation even though dropped above): the test files just excluded (evidence a `Test it` sub task landed) and tooling/config changes (`test-preferences.json`, lint/format config, pre-commit config, CI workflow files) as evidence a tooling sub task landed.
...
**Stop only when `AGENTS_SOURCE_FILES`, `SCOPE_EVIDENCE_FILES`, and dependency manifest changes are all empty** (only lock files or generated output changed): nothing to sync, do not spawn. A change that only adds tests or only touches tooling config has no AGENTS.md line to add but does have `SCOPE_EVIDENCE_FILES`, so it must still continue to Step 2 onward for Step 6 to reconcile the `Test it` / tooling sub tasks that evidence proves.
...
The inputs to apply:
  1. `MODE`, `BASE`, `MERGE_BASE`, `CHANGED_FILES` (`AGENTS_SOURCE_FILES` from Step 1, name status), `DIFF_COMMAND` (exact `git diff` command)
  2. `DELETED_PATHS` (deleted paths, for orphan cleanup)
  3. `SCOPE_EVIDENCE_FILES` (test/tooling changes kept aside in Step 1; not durable conventions, but Step 6 needs them as sub task evidence even on a run with no `AGENTS_SOURCE_FILES` at all)
  4. `ROOT_AGENTS_MD` (root AGENTS.md contents), `NESTED_PATHS` (nested AGENTS.md paths)
  5. `SPEC_PATHS` (all spec paths, for Status line reconciliation and staleness flagging)
  6. `FILE_TO_CONTEXT_MAP` (changed file → nearest context file)
  7. `SCOPE_PATH_OR_NONE` (relevant workspace scope path(s), not all of `docs/scope/`; also the source of each linked feature's status for spec Status line reconciliation)
  8. `INSTALLED_SKILLS_OR_NONE`, `MCP_SERVERS_OR_NONE`, `DECLINED_TOOLS_OR_NONE` from Step 2.5
