...
PR text, `CHANGELOG.md`, `docs/releases/`, `docs/postmortems/` (owned by this skill). Explicit exception, closing gate tick only, on a file `/scope` owns: after producing the document, tick the feature's `Document it` box in its `docs/scope/` row, one existing checkbox flipped, never new prose or a rewrite; see the closing gate in Step 4. Writes nothing else, and never any other line in `docs/scope/`.
...
```bash
# base branch: resolve the repository's real default branch, don't assume main or master
git symbolic-ref --short refs/remotes/origin/HEAD   # strip the origin/ prefix; local cache, works offline
# empty, or no origin remote: fall back to `git remote show origin`, read its "HEAD branch:" line (hits network)
# still nothing (no remote at all): `git config init.defaultBranch`, else the first of `main` / `master`
#   for which `git rev-parse --verify <name>` succeeds
# current branch
git rev-parse --abbrev-ref HEAD

# pr / changelog: the branch change set. Substitute the branch name you resolved above for every
# literal BASE below (it may be `main`, `master`, `develop`, `trunk`, anything) before running these:
...
**Tick the scope box (closing gate).** If the documented feature has a row in `docs/scope/`, tick its `Document it` box (the doc was written; the box marks that) and confirm it in the report: "Scope: ticked `Document it`." No matching row → say so ("no scope row matched `<feature>`, tick it manually or enroll it"). This is the one, narrow exception to "writes nothing else" in Artifact ownership above: a single existing checkbox flipped in a file `/scope` owns, never new prose, never any other line in that file.

This skill does not commit, push, or merge; it produces the prose (and ticks the `Document it` box per the closing gate just above, the only scope edit it makes).
