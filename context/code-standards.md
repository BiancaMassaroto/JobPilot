...
**Exception — redirect-only actions:** `signInWithOAuthAction` and `signOutAction` in `actions/auth.ts` return `Promise<never>` instead of `{ success, error? }`. Both end unconditionally in `redirect()`, which itself always throws (typed `never` by Next.js) — there is no code path where either action returns a value to a caller, so the standard result shape doesn't apply. This is the only sanctioned exception; any new Server Action that can return normally still follows the `{ success, error? }` shape above.
...
| `oauth_sign_in_started` | Google/GitHub button clicked on login page                    | provider                    |
| `marketing_cta_clicked` | Homepage CTA clicked (Get Started / Find Your First Match)    | destination                 |
...
`oauth_sign_in_started` and `marketing_cta_clicked` fire client-side before the user is known, so they carry no `userId` — PostHog attaches them to the anonymous distinct ID, which `identifyUser()` (see the PostHog section in `library-docs.md`) later merges into the real person once the session resolves.
