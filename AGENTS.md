...
- **EXTRA IMPORTANT**: This project intentionally runs Tailwind CSS v4 (decided in Feature 01 — see `context/ui-tokens.md` and `context/ui-rules.md` for the `@theme`-based token system this implies). The generic InsForge template guidance above defaults to pinning 3.4; that default does not apply here. Do not downgrade to v3.4 or introduce a `tailwind.config.ts` for colors/tokens.
